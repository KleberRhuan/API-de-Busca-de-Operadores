...
class TestCORSConfiguration:
    ...
    def test_cors_expose_headers(self, client):
        ...
        for header in required_headers:
            assert header in exposed_headers, f"Cabeçalho {header} não está exposto no CORS"
    ...
    def test_cors_documentation_endpoints(self, client):
        ...
        for endpoint in doc_endpoints:
            ...
            # Verificar se a resposta tem os cabeçalhos CORS esperados
            assert response.status_code == status.HTTP_200_OK, f"Endpoint {endpoint} retornou status {response.status_code}"
            assert "access-control-allow-origin" in response.headers, f"Endpoint {endpoint} não tem cabeçalho CORS"
            assert response.headers["access-control-allow-origin"] == "localhost", f"Endpoint {endpoint} tem origem incorreta" 