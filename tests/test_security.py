...
class TestSecurityControls:
    ...
    def test_rate_limiting(self, client, mock_operator_service, paginated_operators_response):
        ...
        for i in range(num_requests):
            ...
            if i < 10:  # As primeiras 10 requisições devem ser bem-sucedidas
                ...
                assert remaining == 9 - i, f"Contador de rate limit incorreto: {remaining} para requisição {i+1}"
            else:  # As requisições subsequentes devem ser bloqueadas
                assert response.status_code == status.HTTP_429_TOO_MANY_REQUESTS, f"Requisição {i+1} não retornou 429"
    ...
    def test_xss_protection(self, client, mock_operator_service):
        ...
        assert escaped_content.strip('"') in response_text  # O JSON deve ter escapado corretamente
