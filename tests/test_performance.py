...
class TestApiPerformance:
    ...
    def test_operators_endpoint_response_time(
        ...
        assert (
            avg_response_time < max_acceptable_avg
        ), f"Tempo médio de resposta muito alto: {avg_response_time:.2f} ms"
        assert (
            max_response_time < max_acceptable_max
        ), f"Tempo máximo de resposta muito alto: {max_response_time:.2f} ms"

    @pytest.mark.skip("Teste de performance com cache requer configuração adicional")
    @pytest.mark.performance
    def test_cache_performance(self, client):
        """Teste para medir o desempenho do cache"""
        with (
            patch("src.infra.database.cache.get") as mock_cache_get,
            patch("src.infra.database.cache.set") as mock_cache_set,
            patch(
                "src.application.service.operator_service.OperatorService.find_all"
            ) as mock_find_all,
        ):
            ...
            ), "O acesso com cache não é mais rápido que sem cache"
    ...
    @pytest.mark.performance
    @pytest.mark.skip(
        "Teste de carga deve ser executado separadamente em um ambiente controlado"
    )
    def test_load_performance(
        ...
        ), f"Tempo P95 muito alto: {p95_response_time:.2f} ms"
