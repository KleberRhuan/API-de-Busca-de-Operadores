...
class RateLimitMiddleware(BaseHTTPMiddleware):
    def __init__(
        ...
        self.window = window
        logger.info(
            f"Rate limiter inicializado: limite={limit} por {window}s, storage={type(storage).__name__}"
        )
    ...
    async def dispatch(
        ...
        if not is_allowed:
            ...
            logger.info(
                f"Rate limit excedido: {client_ip} em {endpoint}, reset em {reset_in}s"
            )

            raise RateLimitExceededException(
                limit=self.limit, window=self.window, reset_in=reset_in
            )
